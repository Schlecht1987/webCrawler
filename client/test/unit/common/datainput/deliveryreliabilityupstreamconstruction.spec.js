describe("DataInput delivery reliability upstream constrution ", function() {
    ...
    beforeEach(inject(function($controller, $rootScope, _$httpBackend_, _$location_) {
        ...
        ctrl = $controller('DataInputDeliveryReliabilityUpstreamConstructionController', {
            $scope: $scope
        });
        $httpBackend.whenGET("/deliveryreliabilityupstreamconstruction/inputdata/").respond(200, {
            calendarWeeks: [1, 2, 3]
        });
        $httpBackend.flush();
    }));
    ...
    describe("default values", function() {
        ...
        it("smiley should set to 2, 2", function() {
            ...
            expect($scope.smiley.updateKey).toBe(13);
        });
        it("datastate should set all to false", function() {
            expect($scope.datastate.deviationInDays).toBe(false);
    ...
    describe("checkDeviationInDaysInput Funktion", function() {
        it("test should be false", function() {
            $scope.data.deviationInDays = "test";
            $scope.checkDeviationInDaysInput();
            expect($scope.datastate.deviationInDays).toBe(false);
        });
        it("55 should be true", function() {
            $scope.data.deviationInDays = "55";
            $scope.checkDeviationInDaysInput();
            expect($scope.datastate.deviationInDays).toBe(true);
        });
        it("43,3 should be true and changed to 433", function() {
            $scope.data.deviationInDays = "43,3";
            $scope.checkDeviationInDaysInput();
            expect($scope.datastate.deviationInDays).toBe(true);
            expect($scope.data.deviationInDays).toEqual("433");
        });
    ...
    describe("submit Funktion", function() {
        it("response data should be test", function() {
            $httpBackend.whenPOST("deliveryreliabilityupstreamconstruction/").respond(200, 'test');
            $httpBackend.whenPOST("smiley/").respond(200, ' ');
            $scope.datastate = {
                deviationInDays: true,
                calendarWeek: true
            };
            $scope.submit("POST");
            $httpBackend.flush();
        ...
        it("updateTemplate should be set to", function() {
            $httpBackend.whenPOST("deliveryreliabilityupstreamconstruction/").respond(304, 'test');
            $httpBackend.whenPOST("smiley/").respond(200, ' ');
            $scope.datastate = {
                deviationInDays: true,
                calendarWeek: true
